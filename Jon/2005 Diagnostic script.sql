...
-- Copyright Glen Berry
-- 
-- PRE-REQUISITIES:
--	SAVE this file with a .sql extention if it is sent as a .txt
-- 	USER running the script must be a member of the sysadmin SQL Server role
--  PLEASE set the output to spool to a text output or file.  Within SQL Server Management Studio;
--		Choose [Tools] / [Options] / [Query Results] then CHANGE the default output to either RESULTS TO TEXT or RESULTS to FILE
--		Choose OK and then open this file in a new query session within SSMS
--		The results should be saved as a rpt file and returned to support via secure means (password protected ZIP archive is acceptable)
USE MSDB
GO
